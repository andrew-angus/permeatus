...
perm = homogenisation(materials=2,D=np.array([1.0,0.1]),S=np.array([1.0,1.1]),vFrac=np.array([0.4,0.6]),\
              C0=1.0,C1=0.0,touts=[0.001,0.05,0.2,2.0,5.0],tstep=0.001,ncpu=8)
...
perm.voigt_bound()
print(perm.D_eff,perm.S_eff,perm.P_eff)
Dv,Sv,Pv = perm.D_eff, perm.S_eff, perm.P_eff

# %%
perm.reuss_bound()
print(perm.D_eff,perm.S_eff,perm.P_eff)
Dr,Sr,Pr = perm.D_eff, perm.S_eff, perm.P_eff
...
# %%
print(np.mean(perm.field['J'][-1]['data'][:,1])/np.mean(perm.field['J'][-2]['data'][:,1]))
